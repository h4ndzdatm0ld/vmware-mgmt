...
terraform_init_plan:
	...
	terraform -chdir=$(base_tform_dir)/$(VSPHERE_VM_NAME) plan --var="vsphere_password=$(VSPHERE_PASSWORD)" --var="vsphere_vm_disk_size=$(VSPHERE_VM_DISK_SIZE)" --var="vsphere_vm_name=$(VSPHERE_VM_NAME)" --var="vsphere_vm_memory=$(VSPHERE_VM_MEMORY)"--var="vsphere_vm_cpus=$(VSPHERE_VM_CPUS)" --out=tfplan
